    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for k in dataset_val.keys():
                ...
                print("Start considering softmax")
                # this needs to go into background processes
                # result = export_prediction_from_softmax(prediction_for_export, properties, self.configuration_manager, self.plans_manager, self.dataset_json, output_filename_truncated, save_probabilities)
                # print(result)
                # exit()
                results.append(
                ...
                if next_stages is not None:
                    for n in next_stages:
                        self.print_to_log_file("Start to predict next stage...")
                        ...
                        try:
                            ...
                            self.print_to_log_file("tmp dataset complete.")
                        ...
                        self.print_to_log_file("Stage in process...")
                        if should_i_save_to_file(prediction, results, segmentation_export_pool):
                            np.save(output_file[:-4] + '.npy', prediction)
                            prediction_for_export = output_file[:-4] + '.npy'
                        else:
                            prediction_for_export = prediction
                        # resample_and_save(prediction, target_shape, output_file, self.plans, self.configuration, properties, self.dataset_json, n)
                        self.print_to_log_file("next stage22222 ...")
                        ...
                        self.print_to_log_file("Almost complete ...")
                    self.print_to_log_file("GetAll next stage done...")    
                self.print_to_log_file("Done!!")
            self.print_to_log_file("Starting to set predictions...")
            self.print_to_log_file(results[0].get)
            _ = [r.get() for r in results]
            self.print_to_log_file("GetAllRes done...")

        if self.is_ddp:
            self.print_to_log_file("Start DDP...")
            dist.barrier()

        if self.local_rank == 0:
            self.print_to_log_file("Start metrics ... ")
